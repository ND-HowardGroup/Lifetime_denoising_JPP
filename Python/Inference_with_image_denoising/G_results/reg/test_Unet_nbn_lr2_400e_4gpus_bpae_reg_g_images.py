...
class DataGenerator(tensorflow.keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            X[i:(i+1), ..., 0] = x_img.squeeze() / max_G - 0.5
            #Y[i:(i+1), ..., 0] = y_label.squeeze() / 255 - 0.5
            Y[i:(i+1), ..., 0] = y_label.squeeze() / max_G - 0.5
...
def test():
    ...
    io.savemat('test_nbn_Estimated_result_G_registered_bpae_512.mat', dict([('test_nbn_Estimated_result_G_registered_bpae_512',test_results)]))
