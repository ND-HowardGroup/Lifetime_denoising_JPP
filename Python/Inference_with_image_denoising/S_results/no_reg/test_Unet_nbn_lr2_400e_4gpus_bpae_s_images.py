...
CSV_path = '/afs/crc.nd.edu/user/v/vmannam/Desktop/Spring20/Jan20/1101/Keras_inference_bpae_cells_S_images/'
class DataGenerator(tensorflow.keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        if self.test == True:
            ...
            elif self.test_set==8:
                df2 = pandas.read_csv(CSV_path + 'test_samples_raw_bpae_g_images.csv') #G_images
            else:
                df2 = pandas.read_csv(CSV_path + 'test_samples_raw_bpae_s_images.csv') #S_images
        # Generate data
        for i, ID in enumerate(list_IDs_temp):
            ...
            #from skimage import io
                ...
                min_S = -0.1
                max_S = 0.6
                img = np.clip(img, min_S, max_S)
                label = np.clip(label, min_S, max_S)
                img = (img+0.1)/0.7 #s range is -0.1 to 0.6 (so add 0.1 range -> 0 to 0.7 and divide by 0.7 to bring 0 to 1 range)
                label = (label+0.1)/0.7 #s range is -0.1 to 0.6 (so add 0.1 range -> 0 to 0.7 and divide by 0.7 to bring 0 to 1 range)
...
def test():
    ...
    io.savemat('test_nbn_Estimated_result_S_not_registered_bpae_512.mat', dict([('test_nbn_Estimated_result_S_not_registered_bpae_512',test_results)]))
