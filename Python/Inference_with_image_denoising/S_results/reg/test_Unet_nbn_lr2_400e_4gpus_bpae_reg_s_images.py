    ...
    def __data_generation(self, list_IDs_temp):
        ...
        if self.test == True:
            if self.test_set==1:
                ...
                df2 = pandas.read_csv(CSV_path + 'test_lifetime_samples_raw_bpae_reg.csv') #lifetime images mouse_kidney
            elif self.test_set==8:
                df2 = pandas.read_csv(CSV_path + 'test_samples_raw_bpae_g_reg_images.csv') #G_images reg
            else:
                df2 = pandas.read_csv(CSV_path + 'test_samples_raw_bpae_s_reg_images.csv') #S_images reg
            # Generate data
        # Generate data
        for i, ID in enumerate(list_IDs_temp):
            ...
            X[i:(i+1), ..., 0] = x_img.squeeze() / max_S - 0.5
            #Y[i:(i+1), ..., 0] = y_label.squeeze() / 255 - 0.5
            Y[i:(i+1), ..., 0] = y_label.squeeze() / max_S - 0.5
...
def test():
    ...
    io.savemat('test_nbn_Estimated_result_S_registered_bpae_512.mat', dict([('test_nbn_Estimated_result_S_registered_bpae_512',test_results)]))
