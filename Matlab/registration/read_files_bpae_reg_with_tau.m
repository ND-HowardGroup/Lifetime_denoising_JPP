...
addpath('/Users/varunmannam/Desktop/Spring20/Research_S20/Jan20/1101/BPAE_sample4/registration/cropped/G_stack');
addpath('/Users/varunmannam/Desktop/Spring20/Research_S20/Jan20/1101/BPAE_sample4/test_results/G_results/reg');
addpath('/Users/varunmannam/Desktop/Spring20/Research_S20/Jan20/1101/BPAE_sample4/registration/cropped/S_stack');
addpath('/Users/varunmannam/Desktop/Spring20/Research_S20/Jan20/1101/BPAE_sample4/test_results/S_results/reg');

addpath('/Users/varunmannam/Desktop/Spring20/Research_S20/Jan20/1101/BPAE_sample4/original_images_reg/registered/cropped/I_stack');
addpath('/Users/varunmannam/Desktop/Spring20/Research_S20/Jan20/1101/BPAE_sample4/original_images_reg/registered/cropped/Tau_stack');
addpath('/Users/varunmannam/Desktop/Spring20/Research_S20/Jan20/1101/BPAE_sample4/original_images_reg/registered/cropped');
...
denoised_g = load('test_nbn_Estimated_result_G_registered_bpae_512.mat');
denoised_g = denoised_g.test_nbn_Estimated_result_G_registered_bpae_512;
denoised_g = permute(denoised_g,[2 3 1]);

denoised_s = load('test_nbn_Estimated_result_S_registered_bpae_512.mat');
denoised_s = denoised_s.test_nbn_Estimated_result_S_registered_bpae_512;
...
target_intensity = Tiff('Avg_I_reg_image_cropped.tif');
...
input_G(:,:,i) = read(x1);
input_G(:,:,i) = mat2gray(input_G(:,:,i),[gmin,gmax]); %0-1

%denoised = imread(file2);
input_S(:,:,i) = read(x2);
input_S(:,:,i) = mat2gray(input_S(:,:,i),[smin,smax]); %0-1

%lifetime read
input_tau(:,:,i) = read(x3); 
input_tau(:,:,i) = double(input_tau(:,:,i)); %0-1
...
input_intensity(:,:,i) = double(input_intensity(:,:,i)); %0-1
...
%till here tau is within 0 to 10nsec
denoised_tau(:,:,i)  = mat2gray(denoised_tau(:,:,i),[tau_min,tau_max]); %normalize to 0-1

pnsr_res_tau(i,:) = calculate_lifetime_norm_PSNR_512(input_tau(:,:,i), target_tau, denoised_tau(:,:,i));
end
...
tau_max_d = 4e-9;
figure(3)
subplot(1,3,1), imagesc(input_tau(:,:,max_ind)*10e-9,[tau_min_d,tau_max_d]);
colormap('gray'), colorbar;
title('noisy input lifetime')
set(gca,'FontSize',font);
subplot(1,3,2), imagesc(denoised_tau(:,:,max_ind)*10e-9,[tau_min_d,tau_max_d])
colormap('gray'), colorbar;
title('denoised lifetime')
set(gca,'FontSize',font);
subplot(1,3,3), imagesc(target_tau*10e-9,[tau_min_d,tau_max_d])
colormap('gray'), colorbar;
title('target lifetime')
set(gca,'FontSize',font);
...
%assign last values
noisy_input_lt = input_tau(:,:,max_ind)*10e-9;
denoised_lt = denoised_tau(:,:,max_ind)*10e-9;
target_lt = target_tau*10e-9;
...
noisy_input_int = mat2gray(noisy_input_int,[0.3955,0.55]); 
%adjusted brighness manually (0.6 to 0.85 but 98.5% within 0.4 and 0.55)
...
%to tif image
save_to_tiff(rgb1, 'noisy_lifetime_rgb_image1.tif'); %here intensity is not denoised
save_to_tiff(rgb2, 'denoised_lifetime_rgb_image1.tif'); %here intensity is not denoised
save_to_tiff(rgb3, 'target_lifetime_rgb_image1.tif'); %here intensity is not denoised

%to png image
imwrite(rgb1, 'noisy_lifetime_rgb_image_png1.png'); %here intensity is not denoised
imwrite(rgb2, 'denoised_lifetime_rgb_image_png1.png'); %here intensity is not denoised
imwrite(rgb3, 'target_lifetime_rgb_image_png1.png');
% 
% 
